...
export default function HospitalListPage() {
  ...
  useEffect(() => {
    fetchHospitalIndex().then((data: HospitalMaster[]) => {
      // 精神科病院（精神病床70%以上）は精神科ページに集約するため除外
      const nonPsych = data.filter((h) => {
        const psychBeds = h.psychiatricBeds || 0;
        const total = h.totalBeds || 0;
        return total === 0 || psychBeds / total < 0.7;
      });
      setHospitals(nonPsych);
      setLoading(false);
    });
  }, []);
